...
namespace sat {
  ...
  private:
    ...
    sat::ObjectAllocator* allocators[256];
    
    static inline size_t getAllocatorSize(ObjectAllocator* allocator) {
    ...
    static int _compare_allocator(const void* a, const void* b) {
      return getAllocatorSize(*(ObjectAllocator**)a) - getAllocatorSize(*(ObjectAllocator**)b);
    }
    void check() {
      ...
      for (size = 0; size <= sizeLimit; size++) {
        if (ObjectAllocator* allocator = getAllocator(size)) {
  ...
  public:
    ...
    void registerAllocator(ObjectAllocator* allocator) {
      this->allocators[this->numAllocators++] = allocator;
    }
    sat::ObjectAllocator* getMaxAllocator() {
      return this->allocators[this->numAllocators-1];
    }
    sat::ObjectAllocator* getMinAllocator() {
      return this->allocators[0];
    }
    sat::ObjectAllocator* getAllocator(int size) {
      if (size <= 4096) {
        int index = (size - 1) >> 3;
    ...
    void finalize() {
      qsort(this->allocators, this->numAllocators, sizeof(ObjectAllocator*), &this->_compare_allocator);
      ...
      for (int index = 0; index < smallSizeCount; index++) {
        ...
        while (max_size > curAllocSize && curAllocId < this->numAllocators) {
          if (ObjectAllocator* allocator = this->allocators[++curAllocId]) curAllocSize = getAllocatorSize(allocator);
          else curAllocSize = 0;
        }
